...
def get_domain_details(domain_details, verbose):
    ...
    if verbose > 0:
        print('{:20} {}'.format(
            domain_details['DomainName'],
            domain_details['Nameservers']))
    return validate_nameservers(domain_details['Nameservers'], verbose)

def get_zones(domain_detail, verbose):
    zones = []
    if verbose > 0:
        print('Number of zones found:{}'.format(
            len(domain_detail['HostedZones'])))
    if verbose > 1:
...
if '__main__' == __name__:
    ...
    for domain in domains:
        ...
        domains_hash[domain] = get_domain_details(r53domains.get_domain_detail(
            DomainName=domain), args.verbose)
    r53 = boto3.client('route53', region_name='us-east-1')
