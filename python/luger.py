...
def walk_word(results, word, index, debug=False):
    if len(word) == index:
        if debug:
            print("done with this pass, result is:{}".format(word))
        results.append(word)
        return
    w = word[index]  # just to save typing
    if w in trans_table:
        if debug:
            print("found in trans_table")
        for value in trans_table[w]:
            new_word = word[:index] + value + word[index + 1 :]
            if debug:
                print("value:{} new_word:{}".format(value, new_word))
...
if "__main__" == __name__:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="maximum number of words to permute",
    )
    parser.add_argument(
