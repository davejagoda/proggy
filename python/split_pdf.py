...
# must use the Apple provided python to be able to import CoreGraphics
...
def process_pdf(file, verbose=False):
    if os.path.isfile(file):
        input_doc = CoreGraphics.CGPDFDocumentCreateWithProvider(
            CoreGraphics.CGDataProviderCreateWithFilename(file))
        if input_doc:
            max_pages = input_doc.getNumberOfPages()
            page_number_width = len(str(max_pages))
            print('file:{} has {} pages'.format(file, max_pages))
            for page_num in range(1, max_pages + 1):
                media_box = input_doc.getPage(page_num).getBoxRect(
                    CoreGraphics.kCGPDFMediaBox)
                ...
                new_file = '{}.page{}{}'.format(prefix, str(page_num).zfill(
                    page_number_width), suffix)
                if verbose: print('{} width:{} height:{}'.format(
                        new_file, width, height))
                write_context = CoreGraphics.CGPDFContextCreateWithFilename(
                    new_file, page_rect)
