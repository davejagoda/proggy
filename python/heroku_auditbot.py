...
def get_my_email_address():
    return subprocess.check_output(["heroku", "auth:whoami"]).strip().decode("utf-8")
...
def fix_addon(expected_name, actual_name, verbose):
    ...
    if "Y" == input('press "Y" to fix '):
        subprocess.check_call(["heroku", "addons:rename", actual_name, expected_name])
...
def get_apps(verbose):
    ...
    for line in output.splitlines():
        if (
            line.startswith("===")
            or line.startswith("You have no apps")
            or 0 == len(line)
        ):
            pass
        else:
...
def get_app_info(app, my_email_address, verbose):
    output = subprocess.check_output(["heroku", "apps:info", "-a", app]).decode("utf-8")
    if verbose > 1:
        print(output)
    else:
        results = []
...
def get_addons(app, fix, verbose):
    total = 0.0
    output = subprocess.check_output(["heroku", "addons", "-a", app]).decode("utf-8")
    if verbose > 1:
        print(output)
    for line in output.splitlines():
        if (
            line.startswith("──")
            or line.startswith("Add-on")
            or line.startswith(" ")
            or 0 == len(line)
        ):
            pass
...
if "__main__" == __name__:
    ...
    parser.add_argument(
        "-f", "--fix", action="store_true", help="fix inconsistency if possible"
    )
    parser.add_argument("-o", "--org", help="which organization apps to audit")
    parser.add_argument(
        "-v", "--verbose", action="count", default=0, help="show verbose output"
    )
