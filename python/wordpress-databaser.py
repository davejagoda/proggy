...
def create_wordpress_database(db_name, verbose):
    ...
    db_command = 'GRANT ALL PRIVILEGES ON {0}.* TO "{0}"@"localhost" IDENTIFIED BY "{0}";'.format(
        db_name
    )
    if verbose > 0:
        print(db_command)
...
def process_directory(src, dropDatabase, verbose):
    ...
    for root, dirs, filenames in os.walk(src):
        for dir_name in dirs:
            ...
            db_name = dir_name.replace(
                "-", "_"
            )  # convert '-' to '_' to reduce quoting in MySQL
...
if "__main__" == __name__:
    ...
    parser.add_argument(
        "-s", "--src", required=True, help="directory of unzipped wordpress installs"
    )
    parser.add_argument(
        "--dropDatabase",
        action="store_true",
