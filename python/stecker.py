...
# numSockets is the total number of sockets available, and thus the alphabetSize
...
def recursivePairPicker(remainingAlphabet, numWires, currentChain):
    if 0 == len(remainingAlphabet) or 0 == numWires:
        print('remaining alphabet exhausted or out of wires, current chain: {}'.
              format(currentChain))
        pairSet = set()
    ...
    for pairTuple in itertools.combinations(remainingAlphabet, 2):
        ...
        recursivePairPicker(remainingAlphabet - set(pairTuple), numWires - 1,
                            newChain)
    return(currentChain)
...
#    print(socketNames)
    while numSockets <= maxSockets:
        while numWires * 2 <= numSockets:
            setOfPairs = set()
            print('numSockets: {} numWires: {} availableSockets: {}'.format(
                numSockets, numWires, socketNames[:numSockets]))
            recursivePairPicker(set(socketNames[:numSockets]), numWires, [])
            print('length of set: {} elements of set: {}'.format(
                len(setOfPairs), sorted(setOfPairs)))
            numWires += 1
        numSockets += 1
        numWires = 0
        print()
