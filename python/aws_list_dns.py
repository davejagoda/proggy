...
def print_record_sets(r53record_sets, verbose):
    if verbose > 0:
        print(
            "Number of records found:{}".format(
                len(r53record_sets["ResourceRecordSets"])
            )
        )
...
def print_zones(zones, verbose):
    ...
    for zone in zones["HostedZones"]:
        print(
            "{:20} {} {}".format(
                zone["Name"], zone["Id"], zone["ResourceRecordSetCount"]
            )
        )
        response = boto3.client("route53", region_name="us-east-1")
        print_record_sets(
            response.list_resource_record_sets(HostedZoneId=zone["Id"]), args.verbose
        )
